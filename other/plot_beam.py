...
def plot_modeshape(
    ...
    X, Y = np.meshgrid(np.linspace(0, 8, Z.shape[1]), np.linspace(0, 1, Z.shape[0]))
    X = X + phi[:, :, 0]
    Y = Y + phi[:, :, 1]
...
if __name__ == "__main__":
    ...
    # ic(omegas)
    # ks_rho =100
    # eta = np.exp(-ks_rho * (omegas**2 - omegas[0]**2))
    # a = np.sum(eta)
    # eta = eta / a
    # ic(eta)
    
    # q = np.array([phi0[0][49, 399, 1], phi1[0][49, 399, 1], phi2[0][49, 399, 1], phi3[0][49, 399, 1], phi4[0][49, 399, 1], phi5[0][49, 399, 1]])
    # ic(q)
    # ic(np.dot(eta, q**2))
